...
class PipelineManager(PipelineManagerBase):
    ...
    def run_mutect(self, dependency_id=None):
        ...
        command = f"{self.config['TOOLS']['gatk']} Mutect2 --java-options \"{self.config['DEFAULT']['java_options']}\" --reference {self.config['REFERENCES']['fasta']} --input {self.normal} {input_tumor} --output {self.output_dir}/{self.name}.vcf --native-pair-hmm-threads {self.config['DEFAULT']['threads']} --max-mnp-distance 0 --intervals chr9 --intervals chr17"
